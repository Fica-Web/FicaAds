...
import getCroppedImg from "../../../utils/cropImage/getCroppedImg"; // Function to process cropped image
...
const CoverImageUpload = ({ coverImagePreview, onImageChange, error, onCropComplete }) => {
    ...
    return (
        <div>
            <label className="block text-lightgray font-medium">Cover Image</label>
            <input
                type="file"
                accept="image/*"
                onChange={handleFileChange}
                className="w-full border p-2 rounded-md mt-1 file:bg-gray4 hover:file:bg-lightgray file:text-white file:py-1 file:px-5 file:rounded-lg file:cursor-pointer"
            />
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-5 rounded-lg shadow-lg max-w-lg w-full">
                        ...
                        <div className="mt-4 flex justify-end space-x-3">
                            <button className="bg-gray4 text-white px-4 py-2 rounded-md" onClick={() => setShowCropModal(false)}>
                                Cancel
                            </button>
